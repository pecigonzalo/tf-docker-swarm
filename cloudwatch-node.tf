resource "aws_cloudwatch_metric_alarm" "node_cpu_high" {
  alarm_name          = "${var.name}-tf-${var.environment}-node-cpureservation-high"
  ...
  dimensions {
    AutoScalingGroupName = "${aws_autoscaling_group.swarm-node.name}"
  }

  alarm_description = "Scale up if the cpu reservation is above 70% for 10 minutes"
  alarm_actions     = ["${aws_autoscaling_policy.swarm-node-scale-up.arn}"]
...
resource "aws_cloudwatch_metric_alarm" "node_memory_high" {
  alarm_name          = "${var.name}-tf-${var.environment}-node-memoryreservation-high"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "2"
...
resource "aws_cloudwatch_metric_alarm" "node_cpu_low" {
  alarm_name          = "${var.name}-tf-${var.environment}-node-cpureservation-low"
  ...
  alarm_actions     = ["${aws_autoscaling_policy.swarm-node-scale-down.arn}"]
...
resource "aws_cloudwatch_metric_alarm" "node_memory_low" {
  alarm_name          = "${var.name}-tf-${var.environment}-node-memoryreservation-low"
  comparison_operator = "LessThanOrEqualToThreshold"
  evaluation_periods  = "2"
  metric_name         = "MemoryReservation"
  namespace           = "AWS/EC2"
